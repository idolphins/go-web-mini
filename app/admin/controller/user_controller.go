...
func (uc UserController) GetUsers(c *gin.Context) {
	...
	pkg_response.Success(c, gin.H{"users": dto.ToUsersDto(users), "total": total}, "获取用户列表成功")
}
...
func (uc UserController) ChangePwd(c *gin.Context) {
	...
	// 判断前端请求的密码是否等于真实密码
	err = pkg_util.ComparePasswd(correctPasswd, req.OldPassword)
	if err != nil {
		pkg_response.Fail(c, nil, "原密码有误")
		return
	}
	// 更新密码
	var (
	...
	pkg_response.Success(c, nil, "更新密码成功")
}
...
func (uc UserController) CreateUser(c *gin.Context) {
	...
	if currentRoleSortMin >= reqRoleSortMin {
		pkg_response.Fail(c, nil, "用户不能创建比自己等级高的或者相同等级的用户")
		return
	}
...
func (uc UserController) UpdateUserById(c *gin.Context) {
	...
	if userId == int(ctxUser.ID) {
		...
		if currentRoleSortMin >= minRoleSorts[0] {
			pkg_response.Fail(c, nil, "用户不能更新比自己角色等级高的或者相同等级的用户")
			return
		}

		// 用户不能把别的用户角色等级更新得比自己高或相等
		if currentRoleSortMin >= reqRoleSortMin {
			pkg_response.Fail(c, nil, "用户不能把别的用户角色等级更新得比自己高或相等")
			return
		}
	...
	pkg_response.Success(c, nil, "更新用户成功")
...
func (uc UserController) BatchDeleteUserByIds(c *gin.Context) {
	...
	if funk.Contains(reqUserIds, ctxUser.ID) {
		pkg_response.Fail(c, nil, "用户不能删除自己")
		return
	}
	...
	for _, sort := range roleMinSortList {
		if currentRoleSortMin >= sort {
			pkg_response.Fail(c, nil, "用户不能删除比自己角色等级高的用户")
			return
		}
	}
	...
	pkg_response.Success(c, nil, "删除用户成功")
